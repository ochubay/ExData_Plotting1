...
if(file.exists(datafile)) {
    ...
    startdtFeb01 <- strptime("1/1/2007 00:00:00", "%d/%m/%Y %H:%M:%S")
    ...
    ## printing power usage data into the historgam in the PNG file device
    ## since the range between min and max of Global_active_power is ~4, setting breaks to ~4*2
    hist(poweruse$Global_active_power, breaks=8, col="red", 
